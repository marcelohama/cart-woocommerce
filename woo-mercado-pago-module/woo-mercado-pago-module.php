...
require_once dirname( __FILE__ ) . '/includes/sdk/MercadoPago/Http/HttpRequest.php';
require_once dirname( __FILE__ ) . '/includes/sdk/MercadoPago/Http/CurlRequest.php';
require_once dirname( __FILE__ ) . '/includes/sdk/MercadoPago/RestClient.php';
...
if ( ! class_exists( 'WC_Woo_Mercado_Pago_Module' ) ) :
	...
	class WC_Woo_Mercado_Pago_Module {
		...
		public static function validate_credentials_v0() {
			...
			$restclient = new MercadoPago\RestClient();
			$data = [
				'grant_type' => 'client_credentials',
				'client_id' => $client_id,
				'client_secret' => $client_secret
			];
			$restclient->setHttpParam( 'address', 'https://api.mercadopago.com' );
			$response = $restclient->post( '/oauth/token', ['json_data' => json_encode( $data )]);
			if ( $response['code'] == 200 ) {
				$access_token = $response['body']['access_token'];
				$get_request = $restclient->get( '/users/me?access_token=' . $access_token );
				if ( $get_request['code'] == 200 && isset( $get_request['body']['site_id'] ) ) {
					update_option( '_access_token_v0', $access_token, true );
					update_option( '_test_user_v0', in_array( 'test_user', $get_request['body']['tags'], true ) );
					update_option( '_site_id_v0', $get_request['body']['site_id'], true );
					update_option( '_collector_id_v0', $get_request['body']['id'], true );
					// Get available payment methods.
					$payment_methods = $restclient->get( '/v1/payment_methods/?access_token=' . $access_token );
					$arr = array();
					$arr[] = 'n/d';
					foreach ( $payment_methods['body'] as $payment ) {
						$arr[] = $payment['id'];
					}
					...
					$currency_ratio = WC_Woo_Mercado_Pago_Module::get_conversion_rate(
						WC_Woo_Mercado_Pago_Module::$country_configs[$get_request['body']['site_id']]['currency']
					);
					if ( $currency_ratio > 0 ) {
		...
		public static function validate_credentials_v1() {
			...
			if ( $get_request['code'] == 200 && isset( $get_request['body']['site_id'] ) ) {
				...
				update_option( '_test_user_v1', in_array( 'test_user', $get_request['body']['tags'] ), true );
				update_option( '_site_id_v1', $get_request['body']['site_id'], true );
				update_option( '_collector_id_v1', $get_request['body']['id'], true );
				// Get available payment methods.
				$payments = $restclient->get( '/v1/payment_methods/?access_token=' . $access_token );
		...
		public static function get_conversion_rate( $used_currency ) {
			$restclient = new MercadoPago\RestClient();
			$restclient->setHttpParam( 'address', 'https://api.mercadopago.com' );
			$currency_obj = $restclient->get(
				'/currency_conversions/search?' .
				'from=' . get_woocommerce_currency() .
				'&to=' . $used_currency
			);
			if ( isset( $currency_obj['body'] ) ) {
				$currency_obj = $currency_obj['body'];
		...
		public static function get_categories() {
			...
			foreach ( $currency_obj['body'] as $category ) {
				array_push(
