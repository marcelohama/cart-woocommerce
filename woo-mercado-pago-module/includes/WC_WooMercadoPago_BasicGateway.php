...
require_once dirname( __FILE__ ) . '/sdk/MercadoPago/Http/HttpRequest.php';
require_once dirname( __FILE__ ) . '/sdk/MercadoPago/Http/CurlRequest.php';
require_once dirname( __FILE__ ) . '/sdk/MercadoPago/RestClient.php';
...
class WC_WooMercadoPago_BasicGateway extends WC_Payment_Gateway {
	...
	public function __construct() {
		...
		$this->restclient = new MercadoPago\RestClient();
		...
		// Find two cards mode value.
		$this->restclient->setHttpParam( 'address', 'https://api.mercadopago.com' );
		$two_cards_info = $this->restclient->get(
			'/account/settings?access_token=' . get_option( '_access_token_v0', '' )
		);
		if ( $two_cards_info['code'] == 200 ) {
			$this->settings['two_cards_mode'] = ( $two_cards_info['body']['two_cards'] == 'active' ? 'yes' : 'no' );
			update_option(
				$this->get_option_key(),
	...
	public function custom_process_admin_options() {
		...
		$is_two_cards_mode = false;
		foreach ( $this->get_form_fields() as $key => $field ) {
			if ( 'title' !== $this->get_field_type( $field ) ) {
				if ( $key == 'two_cards_mode' ) {
					// Get checkbox value and set external value.
					$value = $this->get_field_value( $key, $field, $post_data );
					$is_two_cards_mode = ( $value == 'yes' ? true : false );
					$this->restclient->setHttpParam( 'address', 'https://api.mercadopago.com' );
					$two_cards_info = $this->restclient->put(
						'/account/settings?access_token=' . get_option( '_access_token_v0', '' ),
						array(
							'headers' => array(
							...
							'json_data' => array(
								'two_cards' => ( $is_two_cards_mode ? 'active' : 'inactive' )
							)
						)
					);
					// Set checkbox local value by checking its external new value.
					if ( $two_cards_info['code'] == 200 ) {
						$this->settings[$key] = $value;
						$is_two_cards_mode = ( $two_cards_info['body']['two_cards'] == 'active' ? true : false );
					}
		...
		if ( ! empty( $_site_id_v0 ) ) {
			...
			$infra_data['two_cards'] = ( $is_two_cards_mode ? 'true' : 'false' );
			$this->restclient->setHttpParam( 'address', 'https://api.mercadopago.com' );
			$two_cards_info = $this->restclient->post(
				'/modules/tracking/settings?access_token=' . get_option( '_access_token_v0', '' ),
				array(
					'json_data' => array(
						'two_cards' => $infra_data
					)
				)
			);
			$this->write_log( __FUNCTION__, 'infra data: ' . json_encode( $infra_data, JSON_PRETTY_PRINT|JSON_UNESCAPED_UNICODE ) );
		}
		// Apply updates.
